...
export function renderChat(props: ChatProps) {
  ...
  const thread = html`
    ...
    >
      ${props.loading ? html`<div class="muted">正在加载聊天记录…</div>` : nothing}
      ...
      ${props.queue.length
        ? html`
            <div class="chat-queue" role="status" aria-live="polite">
              <div class="chat-queue__title">队列中 (${props.queue.length})
        ...
        <div class="chat-compose__row">
          ...
          <div class="chat-compose__actions">
            ...
            <button
              ...
              @click=${props.onSend}
            >
              ${isBusy ? "加入队列" : "发送"}<kbd class="btn-kbd">↵</kbd>
            </button>
          </div>
        </div>
      </div>
...
function buildChatItems(props: ChatProps): Array<ChatItem | MessageGroup> {
  ...
  if (historyStart > 0) {
    items.push({
      ...
      message: {
        role: "system",
        content: `显示最近 ${CHAT_HISTORY_RENDER_LIMIT} 条消息 (${historyStart} 条已隐藏)。`,
        timestamp: Date.now(),
      },
    });
  }
